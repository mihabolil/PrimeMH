...
use crate::memory::{structs::{StatValueStruct, Unit, StatsList}, process::D2RInstance};
use convert_case::{Case, Casing};
use super::skills::{Skill, get_skill_class, SkillTree};
    ...
    for n in 0..stat_list.stat_count {
        if stat_list.stat_ptr > 0 {
            ...
            if stat_struct.stat == 0 && stat_struct.value == 0 {
                // println!("stat_struct.stat {:?} unit {:?}", stat_struct.stat, &unit);
                break;
            }
            let this_stat = Stat::new(&mut stat_struct);
...
#[allow(unused)]
impl Stat {
    pub fn new(raw: &mut StatValueStruct) -> Self {
        let stat_name = StatEnum::from_u16(raw.stat).unwrap_or_default();
        match stat_name {
        ...
        // println!("{} {} {:?} {}", raw.layer, raw.stat, stat_name, raw.value);
        Stat {
            layer: raw.layer,
            stat: stat_name,
            value: raw.value,
    ...
    pub fn format_stat(&self, player_level: i16) -> StatFormat {
        match self.stat {
            StatEnum::Strength => StatFormat { stat_str: format!("+{} to Strength", self.value), order: 67},
            StatEnum::Energy => StatFormat { stat_str: format!("+{} to Energy", self.value), order: 61},
            StatEnum::Dexterity => StatFormat { stat_str: format!("+{} to Dexterity", self.value), order: 65},
            StatEnum::Vitality => StatFormat { stat_str: format!("+{} to Vitality", self.value), order: 63},
            ...
            StatEnum::Defense => StatFormat { stat_str: format!("+{} Defense", self.value), order: 71},
            ...
            StatEnum::FireResist => StatFormat { stat_str: format!("Fire Resist +{}%", self.value), order: 36},
            StatEnum::MaxFireResist => StatFormat { stat_str: format!("+{}% to Maximum Fire Resist", self.value), order: 42},
            StatEnum::LightningResist => StatFormat { stat_str: format!("Lightning Resist +{}%", self.value), order: 38},
            StatEnum::MaxLightningResist => StatFormat { stat_str: format!("+{}% to Maximum Lightning Resist", self.value), order: 43},
            StatEnum::ColdResist => StatFormat { stat_str: format!("Cold Resist +{}%", self.value), order: 40},
            StatEnum::MaxColdResist => StatFormat { stat_str: format!("+{}% to Maximum Cold Resist", self.value), order: 44},
            StatEnum::PoisonResist => StatFormat { stat_str: format!("Poison Resist +{}%", self.value), order: 34},
            ...
            StatEnum::AddSkillTab => {
                ...
                StatFormat { stat_str: format!("+{} to {} Skills", self.value, skill_tab), order: 151}
            },
...
pub fn format_stat_list(stats: &Vec<Stat>, player_level: i16) -> Vec<String> {
    let formatted_hs: HashSet<StatFormat> = stats.iter().map(|stat| stat.format_stat(player_level)).collect();
    let mut formatted: Vec<StatFormat> = formatted_hs.into_iter().collect();
    formatted.retain(|x| !x.stat_str.ends_with("Defense"));  // remove defense
